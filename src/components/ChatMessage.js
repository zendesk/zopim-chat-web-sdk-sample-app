...
class ChatMessage extends Component {
    ...
    return (
      <div>
        {options.map((option, i) => {
          return (
            <div>
              <input type="radio" name="option" value={i} onChange={this.optionOnChange} /> {option}
            </div>
          )
        })}
      </div>
  ...
  render() {
    return (
      <div
        className={`chat-msg-container ${this.getClassName(this.props.message)} ${
          this.props.addClass
        }`}
      >
        ...
        <div className="chat-msg-wrapper">{this.renderMessagePart(this.props.message)}</div>
      </div>
    )
  }
}
